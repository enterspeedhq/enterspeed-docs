...
Do this in **every affected environment**. Environments are processed independently, so a Published environment with its own data source needs its own schema deployment.
