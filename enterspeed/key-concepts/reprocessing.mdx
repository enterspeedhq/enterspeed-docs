...
## Configuration changes that require reprocessing

Dependencies aren't the only reason to reprocess. Some configuration is applied while your content is being processed, which means a change only affects content that is processed *after* the change. Existing views keep what they were given when they were processed.

### Adding or changing a domain or hostname
...
Adding the domain afterwards does not create the missing routes. Nothing rebuilds routes for content that has already been processed. So whenever you:

- add a new hostname,
- move a hostname to a different domain, or
- register a domain for a site whose content is already in Enterspeed,

the views that were already processed keep the routes they were originally given, and any URL route that was skipped stays missing.

To pick up the change:

1. Register the hostname(s) on a domain first.
...
Repeat step 2 in **every affected environment**. A domain is registered once for your tenant, but content is processed per environment, so a Published environment with its own data source needs its own schema deployment.
