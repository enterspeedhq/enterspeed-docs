...
| **Sent as** | `x-api-key` header | `x-api-key` header | — |
...
That means the key you issue is the whole boundary. A key scoped to one environment and one index prefix gives an agent exactly that much and no more.

<Warning>
**Custom connectors on claude.ai in the browser are not supported yet.** Browser-based custom connectors cannot send a custom HTTP header, and both tenant-facing servers require `x-api-key`. Use Claude Code, Claude Desktop, VS Code, Cursor, or your own agent instead.
