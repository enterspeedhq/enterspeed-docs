...
Read more about [reprocessing](/enterspeed/key-concepts/reprocessing.md).

### Exclude properties from lookup if you don't need them
By default when you do a [lookup](/enterspeed/reference/js/full-schema/properties#lookup) in a schema, the entire source entities are loaded from storage, including the `properties` object. 
