...
<Steps>
  ...
  <Step title="Enterspeed pushes it out to Speedtrain" icon="arrow-up-right-from-square">
    ...
    The view can also carry an `actions` array naming the task configurations to run, so Enterspeed decides not just *what* crosses over but *what happens to it*. Send the data on its own instead, and Speedtrain simply indexes it until someone starts a session.
