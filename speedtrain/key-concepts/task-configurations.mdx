...
A configuration is versioned, and exactly one version is **active** at a time.
...
<Steps>
  ...
  <Step title="Activate">
    Publishing makes the version live. Activation names the version it displaces, so rollback is a single action.
  </Step>
</Steps>

<Warning>
  Never let activation be the first time a version runs. Preview first.
</Warning>
